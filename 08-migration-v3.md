...
本页把历史行为集中在一起，帮助你更新旧脚本。目标版本是 `v3.6.0`。

## v3.6.0 更新

新增知乎、CSDN、头条未发布草稿流程：CLI 本地准备正文，浏览器 Agent 在已登录账号中保存并重新打开核对。准备成功不等于草稿完成，不包含公开发布；详见[多平台草稿教程](11-multi-platform.md)。

<!-- historical:start -->
## v3.5.0 更新

新增 Atlas Cloud 图片服务；TuZi 为既有服务。排版数量仍为 48 / 77 / 56 / 63，现有正确排版不需要因本次图片服务升级重写。配置前运行 `md2wechat providers show atlascloud --json`，详见[图片教程](05-ai-image.md)。
